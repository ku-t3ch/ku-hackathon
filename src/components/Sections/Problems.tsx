...
const Problems: NextPage<Props> = () => {
  ...
  const charts: CollapseProps['items'] = IssueTree.map((t, idx) => {
    return {
      key: `Collapsible ${t.name}`,
      label: <div className="text-xl md:text-2xl">{t.name}</div>,
      children: (
        <div key={`callapse-${idx}`} className="flex flex-col gap-3">
          {_.orderBy((t as TreeNode).children, (o) => o.value, 'desc').map(
            (item) => (
              <Card key={`collapse-card-${idx}`} className="w-full">
