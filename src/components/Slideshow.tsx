...
export default function Slideshow() {
  ...
  const arrowStyle: string = 'z-20 hidden md:block absolute top-[50%] -translate-x-0 -translate-y-[-50%] text-2xl p-2 text-white cursor-pointer';
  ...
  const overlayStyle = {
    backgroundImage: `linear-gradient(rgba(0, 0, 0, 1), rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 1)), url(${
      slides[currIndex].url
    })`
  };

  return (
    <div className='h-screen w-full flex'>
      <div style={overlayStyle} className='w-full h-full bg-center bg-cover flex flex-col justify-center'>
        <div className='z-10'>
          <img
              src="home-title.png"
          ...
          {slides.map((slide, index) => (
            <div className='cursor-pointer'>
              <RxDotFilled 
                ...
                className={`mx-1 ${currIndex === index ? 'text-[#0DBC58]' : 'text-white'}`} 
                size={30}
              />
            </div>
          ))}
