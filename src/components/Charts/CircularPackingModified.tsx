...
export const CircularPacking = ({
  ...
  return (
    <svg width={width} height={height} style={{ display: 'inline-block', overflow: 'visible' }}>
      {root
        .descendants()
        .slice(1)
        .map((node) => (
      ...
      {root
        ...
        .map((node) => {
          ...
          return (
            ...
            >
              <span style={{
                fontSize: hover == node.data.name ? fontSize: 0,
                fontWeight: 0.4,
                color: "#fff",
              }} className='duration-300 ease-in-out'>{node.data.name}</span>
            </foreignObject>
          );
        })}
    </svg>
